...
namespace TheApplication.Model
{
    class LuceneHelper
    {
        ...
        private void IndexDocument(SEDocument document)
        {
            ...
            Field authorField = new Field(AUTHOR_FN, document.Author, Field.Store.YES, Field.Index.ANALYZED, Field.TermVector.YES);
            Field bibliographicField = new Field(BIBLIOGRAPHIC_FN, document.Bibliographic, Field.Store.YES, Field.Index.ANALYZED, Field.TermVector.YES);
        ...
        public List<SEDocument> SearchText(string querytext)
        {
            List<SEDocument> resultDocs = new List<SEDocument>();
            if (!string.IsNullOrEmpty(querytext))
            {
                querytext = querytext.ToLower();
                Query query = _MultiFieldQueryParser.Parse(querytext);
                ...
                foreach (ScoreDoc scoreDoc in results.ScoreDocs)
                {
                    ...
                    resultDocs.Add(new SEDocument(doc.Get(DOCUMENTID_FN).ToString(),
                        doc.Get(TITLE_FN).ToString(),
                        doc.Get(AUTHOR_FN).ToString(),
                        doc.Get(BIBLIOGRAPHIC_FN).ToString(),
                        doc.Get(ABSTRACT_FN).ToString()));
                }
            }
            return resultDocs;
        }
